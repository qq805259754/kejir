科技热博客 - 生产部署上线完整流程
手动上传文件部署，适用于 WinSCP / FileZilla / Xftp 等 SFTP 工具。 本文档基于以下部署约定编写：
...
4.3.1 推荐上传顺序
先上传配置文件到 /usr/local/nginx/kejir/：docker-compose.yml、Dockerfile、Dockerfile.nginx、deploy.sh、nginx-main.conf、nginx-kejir.conf、package.json、package-lock.json、.env.production
再上传 SSL 证书到 /usr/local/nginx/kejir/ssl/：kejir.com_bundle.pem、kejir.com.key
最后上传 dist/ 目录到 /usr/local/nginx/kejir/（文件较多，耗时最长）
4.3.2 上传后的服务器目录结构
上传完成后，服务器目录结构应为：
...
检查项目目录是否存在
检查 Docker / Docker Compose 是否已安装
检查 selfnet 网络是否存在（不存在则自动创建）
检查 /usr/local/nginx/kejir/ssl/ 下证书文件是否齐全
...
构建 SSR 镜像（基于 Dockerfile，安装 Node.js 22 依赖，复制 dist/server）
构建 Nginx 镜像（基于 Dockerfile.nginx，复制静态资源和 SSL 证书到 /usr/local/nginx）
创建 kejir-net 网络
启动 kejir-ssr 和 kejir-nginx 容器
...
本地重新构建：
npm run build
使用 SFTP 工具上传新的 dist/ 目录到 /usr/local/nginx/kejir/dist/（覆盖旧文件）

如果修改了 Dockerfile、Dockerfile.nginx、docker-compose.yml 或 Nginx 配置文件，也一并上传
...
SFTP 上传新的配置文件到 /usr/local/nginx/kejir/
重建 Nginx 容器：
cd /usr/local/nginx/kejir
...
SFTP 上传新的 dist/client/ 到 /usr/local/nginx/kejir/dist/client/（覆盖旧文件）
重建 Nginx 容器：
cd /usr/local/nginx/kejir
docker compose up -d --build nginx
...
SFTP 上传新的证书文件到 /usr/local/nginx/kejir/ssl/，覆盖旧文件
重新部署（如果已上传 deploy.sh，推荐直接运行脚本）：
cd /usr/local/nginx/kejir
./deploy.sh
或手动重建 Nginx 容器：
...
docker exec kejir-nginx ls -la /usr/local/nginx/ssl
九、常见问题排查
问题 1：登录报 "Required request body is missing"
原因：SSR middleware 未正确转发 POST 请求体。
...
如果不是 stream-duplex，说明 dist/ 未更新，需本地重新构建并上传
确认本地 Node.js 版本 >= 22.12.0
确认 dist/server/virtual_astro_middleware.mjs 包含 context.request.body 和 duplex: 'half' 逻辑
问题 2：Nginx 容器启动失败 "pwrite() failed"
原因：CentOS 7 SELinux 限制。

排查：

# 查看 SELinux 状态
getenforce
# 如果是 Enforcing，可能是 SELinux 拦截
解决：

docker-compose.yml 已配置 privileged: true + tmpfs。如果仍报错，临时关闭 SELinux：
...
# 确认 SSR 容器在 selfnet 网络上
docker inspect kejir-ssr --format '{{json .NetworkSettings.Networks}}' | python -m json.tool

# 从 SSR 容器内测试后端连通性
docker exec kejir-ssr wget -qO- http://blog:8080/kj/api/webConfig

# 检查后端容器是否在 selfnet 网络上
docker network inspect selfnet --format '{{range .Containers}}{{.Name}} {{end}}'
解决：

如果 selfnet 不存在，创建它：docker network create selfnet
如果 blog 容器未接入 selfnet，重启后端容器并接入该网络
问题 4：SSL 证书错误
排查：
...
证书文件名与 nginx-kejir.conf 配置不一致
证书链缺失
私钥有密码保护
证书已过期
问题 5：文件上传 413 错误
nginx-kejir.conf 已设置 client_max_body_size 20m。如需更大限制：

修改 nginx-kejir.conf 中的 client_max_body_size 值
上传新配置到服务器
重建 Nginx 容器：
cd /usr/local/nginx/kejir
docker compose up -d --build nginx
问题 6：Nginx 容器内文件权限问题
排查：

# 进入 Nginx 容器查看目录权限
docker exec -it kejir-nginx ls -la /usr/local/nginx/
docker exec -it kejir-nginx ls -la /usr/local/nginx/ssl/

# 确保证书可被 nginx 用户读取
docker exec kejir-nginx sh -c "su -s /bin/sh nginx -c 'cat /usr/local/nginx/ssl/kejir.com.key'"
解决：

正常情况下 Dockerfile.nginx 会自动设置权限
如果手动修改过，可重建 Nginx 镜像恢复默认权限
问题 7：本地构建成功但服务器启动失败
排查步骤：

确认上传的 dist/ 是最新的
确认服务器上 package.json 和 package-lock.json 已上传
查看 SSR 容器日志：
docker compose logs -f ssr
常见原因：
Node.js 版本不兼容
依赖安装失败（网络问题）
dist/server/entry.mjs 缺失
问题 8：DELETE / PUT 请求返回 403
可能原因：

Nginx 或上游安全策略拦截了非 GET/POST 方法
后端鉴权失败（token 过期、权限不足）
OPTIONS 预检请求未正确响应，导致浏览器未发送实际请求
排查：

# 查看 Nginx 访问日志，确认 403 是由 Nginx 返回还是后端返回
docker exec kejir-nginx tail -n 50 /usr/local/nginx/logs/access.log

# 查看 Nginx 错误日志
docker exec kejir-nginx tail -n 50 /usr/local/nginx/logs/error.log

# 查看 SSR 容器日志，确认 middleware 转发状态
docker compose logs -f ssr
解决：

确认已使用最新 nginx-kejir.conf 和 src/middleware.ts（已配置 OPTIONS 预检响应并显式允许 GET/POST/PUT/DELETE/PATCH/OPTIONS/HEAD）
重新构建并部署 Nginx 与 SSR 容器：
cd /usr/local/nginx/kejir
docker compose up -d --build
如果 403 仍由后端返回，检查后端日志中的鉴权/权限错误
问题 9：删除评论/文章时报 "Cross-site DELETE form submissions are forbidden"
原因：Astro 7 默认开启 security.checkOrigin，会对非 GET 请求进行 origin 校验。本项目所有 API 通过 Nginx -> SSR -> 后端同源代理，浏览器 origin 为 https://kejir.com，而 SSR 容器内部看到的 url.origin 是 http://ssr:4321，导致 Astro 误判为跨站请求并返回 403。

排查：

# 查看响应体是否包含该错误文本
curl -s -X DELETE https://kejir.com/protal/user/delMyComment/1 \
  -H "Authorization: Bearer 你的token" | head -c 100
解决：

已在 astro.config.mjs 中关闭 checkOrigin：
security: {
  checkOrigin: false,
}
重新构建并上传 dist/，然后重建 SSR 容器：
cd /usr/local/nginx/kejir
docker compose up -d --build ssr
关闭该检查是安全的：所有非 GET 请求均通过 fetch 发送，并使用后端 Authorization token 鉴权，不存在跨站表单提交风险
十、文件用途说明
文件	用途
